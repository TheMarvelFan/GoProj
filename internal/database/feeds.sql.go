...
const deleteFeedsFollowedByUserByFeedId = `-- name: DeleteFeedsFollowedByUserByFeedId :exec
DELETE FROM feed_followers WHERE user_id = $1 AND feed_id = $2
`

type DeleteFeedsFollowedByUserByFeedIdParams struct {
	UserID uuid.UUID
	FeedID uuid.UUID
}

func (q *Queries) DeleteFeedsFollowedByUserByFeedId(ctx context.Context, arg DeleteFeedsFollowedByUserByFeedIdParams) error {
	_, err := q.db.ExecContext(ctx, deleteFeedsFollowedByUserByFeedId, arg.UserID, arg.FeedID)
	return err
}
